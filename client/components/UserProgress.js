...
class UserProgress extends Component {
  ...
  sort(criteria) {
    return this.props.results
      .sort((a, b) => {
        return (
          new Date(a.users_books[criteria]) - new Date(b.users_books[criteria])
        )
      })
      .reduce((accum, val) => {
        if (val.users_books[criteria]) {
          let date = val.users_books[criteria]
  ...
  render() {
    ...
    return (
      <div>
        <Typography Typography variant="h5" component="h5" gutterBottom>
          Monthly Book Totals
        </Typography>
        <Divider />
        <BarChart
        ...
        <PieChart width={800} height={400} margin={{top: 30}}>
          <Pie
...
export default withStyles(styles)(connect(mapState)(UserProgress))
